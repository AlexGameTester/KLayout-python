...
class Design8Q(ChipDesign):
    ...
    def draw_xmons_and_resonators(self, res_idx=None):
        ...
        for res_idx, (res, fork_y_span, xmon_dy_Cg_coupling) in it_list:
            ...
            trans_second = DCplxTrans(
                1, 0, False,
                self.coupling_res_length + 3*self.xmon_x_distance, 0
            )
            res.make_trans(trans_second)
            ...
            xmon_current.make_trans(trans_second)
            ...
            if (res_idx is None) or (res_idx == res_idx):
                ...
                self.xmons_corrected.append(xmonCross_corrected)
                xmonCross_corrected.make_trans(trans_second)
                xmonCross_corrected.place(self.region_ph)
    ...
    def draw_readout_waveguide(self):
        ...
        p_last = self.contact_pads[-2].end
        # start of readout
    ...
    def draw_bridges(self):
        ...
        for key, val in self.__dict__.items():
            if "cpwrl_md" in key:
                cpwrl_md = val
                Bridge1.bridgify_CPW(
                    cpwrl_md, bridges_step, dest=self.region_bridges1, dest2=self.region_bridges2,
                    avoid_points=[squid.origin for squid in self.squids]
                    )
            elif "cpwrl_fl" in key:
                cpwrl_fl = val
                Bridge1.bridgify_CPW(
                    cpwrl_fl, fl_bridges_step, dest=self.region_bridges1,
                    dest2=self.region_bridges2,
        ...
        Bridge1.bridgify_CPW(
            self.cpwrl_ro_line1, bridges_step=bridges_step, dest=self.region_bridges1,
            dest2=self.region_bridges2, avoid_points=avoid_points, avoid_distances=avoid_distances
            )
        Bridge1.bridgify_CPW(
            self.cpwrl_ro_line2, bridges_step=bridges_step, dest=self.region_bridges1,
            dest2=self.region_bridges2, avoid_points=avoid_points, avoid_distances=avoid_distances
            )
