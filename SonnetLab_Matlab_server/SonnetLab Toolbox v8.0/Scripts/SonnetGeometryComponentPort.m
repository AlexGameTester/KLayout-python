    ...
    methods
        ...
        function obj = SonnetGeometryComponentPort(theFid)
            ...
            if nargin == 1
                ...
                fgetl(theFid);
                aTempString = '';
                if strcmp(strtrim(fgetl(theFid)), 'EXSMDP')
                    while ~strcmp((aTempString), 'END EXSMDP')
                        aTempString = fgetl(theFid);
                        parts = split(strtrim(aTempString));
                        theIndex = size(obj.Extras, 2)+1;
                        for iCounter = 1:length(parts)
                            obj.Extras{iCounter, theIndex} = parts{iCounter};
                        end
                    end
                else
                    obj.Extras = {};
        ...
        function aSignature=stringSignature(obj,thePortNumber,theVersion) %#ok<INUSD>
            ...
            aSignature = [aSignature newline];
            
            if ~isempty(obj.Extras)
                aSignature = [aSignature sprintf('EXSMDP\n')];
                for iCounter = 1:size(obj.Extras, 2)
                    aSignature = [aSignature obj.Extras{1, iCounter}]; %#ok<AGROW>
                    for jCounter = 2:size(obj.Extras, 1)
                        aSignature = [aSignature ' ' obj.Extras{jCounter, iCounter}]; %#ok<AGROW>
                    end
                    aSignature = [aSignature newline]; %#ok<AGROW>
                end
                %Something else prints the 'END EXSMDP\n' line, I do not know what
            end
