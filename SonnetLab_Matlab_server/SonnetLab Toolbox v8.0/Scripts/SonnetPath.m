...
elseif strcmp(computer,'PCWIN64')
    %Check in registry where 17 should be
    [Status, ~]=system('reg.exe export "HKEY_LOCAL_MACHINE\SOFTWARE\Sonnet Software" "SonnetRegEntry.reg" /y');
    if Status == 1
        %If 17 isn't found, check in old registry location
        [Status, cmdout]=system('reg.exe export "HKEY_LOCAL_MACHINE\SOFTWARE\Sonnet Software\sonnet" "SonnetRegEntry.reg" /y');
        if Status == 1
            % Check if C:\Program Files\sonnet.13.52 exists, if so display a warning and use it instead of throwing an error
            if exist('C:\\Program Files\\sonnet.13.52','dir') ~= 0 
...
aSonnetInstallVersionNumberList={};

% Store the waste character
aRegistryFileLine=fgetl(aFid);
aWasteCharacter=aRegistryFileLine(4);
...
while feof(aFid)==0
    ...
    elseif contains(aRegistryFileLine,'HKEY_LOCAL_MACHINE')
        if ~strcmp('\Sonnet]', aRegistryFileLine(end-7:end))
            aInstalledVersion=strrep(aRegistryFileLine,'[HKEY_LOCAL_MACHINE\SOFTWARE\Sonnet Software','');
            aInstalledVersion=strrep(aInstalledVersion,'\sonnet','');
            aInstalledVersion=strrep(aInstalledVersion,']','');
            if ~isempty(aInstalledVersion)
                aNewNumberOfInstalledVersions=length(aSonnetInstallDirectoryList)+1;
                aSonnetInstallVersionNumberList{aNewNumberOfInstalledVersions}=aInstalledVersion; %#ok<AGROW>
            end
        end
    elseif contains(aRegistryFileLine,'"InstallationDate"=')
...
% If they did not specify a version number use the greatest version
% number.
else
    ...
    for jCounter=1:length(aSonnetInstallVersionNumberList)
        if strcmp(aSonnetInstallVersionNumberList{jCounter},'""')
            continue
        end
        if string(aSonnetInstallVersionNumberList{jCounter}) > aGreatestVersion
            aGreatestVersion=string(aSonnetInstallVersionNumberList{jCounter});
