classdef SonnetGeometryBlock < handle
    ...
    methods
            ...
            if nargin == 1
                ...
                while(isKeepLooping)
                    ...
                    switch aTempString
                        ...
                        case 'STF'
                            aStfLinks{length(aStfLinks)+1} = fscanf(theFid,' %s\n',1);
                        ...
                        case 'VCNSTR'                                                   % Defines the beginning of a VCNSTR sub-block
                            aArrayOfVariableOptions{length(aArrayOfVariableOptions)+1}=...
                                SonnetGeometryVariableOption(theFid);                   % Add specifications for variable optios like interpolation
                ...
                obj.ArrayOfVariableOptions=aArrayOfVariableOptions;
        ...
        function writeObjectContents(obj, theFid, theVersion)
            ...
            if ~isempty(obj.StfLinks)
                for idx = 1:length(obj.StfLinks)
                    fprintf(theFid, 'STF %s\n', obj.StfLinks{idx});
                end
            end
            ...
            for iCounter= 1:length(obj.ArrayOfVariables)
                obj.ArrayOfVariables{iCounter}.writeObjectContents(theFid,theVersion);
                for kCounter= 1:length(obj.ArrayOfVariableOptions)
                    if strcmpi(obj.ArrayOfVariableOptions{kCounter}.VariableName,obj.ArrayOfVariables{iCounter}.VariableName)
                        obj.ArrayOfVariableOptions{kCounter}.writeObjectContents(theFid, theVersion);
                    end
                end
                for jCounter= 1:length(obj.ArrayOfParameters)
                    if strcmpi(obj.ArrayOfVariables{iCounter}.VariableName,obj.ArrayOfParameters{jCounter}.Parname)==1
        ...
        function aSignature=stringSignature(obj,theVersion)
            ...
            if ~isempty(obj.StfLine)
                aSignature = [aSignature obj.StfLine];
            end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function theEquivalenceValue=pointsMatch(obj,theFirstPolygon,theSecondPolygon) %#ok<MANU>
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function boundingBox = getGeometryBoundingBox(obj)
            ...
            boundingBox(1,1) = obj.ArrayOfPolygons{1, 1}.BoundingBox.Start.X;
            boundingBox(1,2) = obj.ArrayOfPolygons{1, 1}.BoundingBox.Start.Y;
            boundingBox(2,1) = obj.ArrayOfPolygons{1, 1}.BoundingBox.End.X;
            boundingBox(2,2) = obj.ArrayOfPolygons{1, 1}.BoundingBox.End.Y;
            for idx = 2:numel(obj.ArrayOfPolygons)
                boundingBox(1,1) = min(obj.ArrayOfPolygons{idx}.BoundingBox.Start.X, boundingBox(1,1));
                boundingBox(1,2) = min(obj.ArrayOfPolygons{idx}.BoundingBox.Start.Y, boundingBox(1,2));
                boundingBox(2,1) = max(obj.ArrayOfPolygons{idx}.BoundingBox.End.X, boundingBox(2,1));
                boundingBox(2,2) = max(obj.ArrayOfPolygons{idx}.BoundingBox.End.Y, boundingBox(2,2));
            end
        end
        ...
        function centerGeometry(obj)
            ...
            %   bounding box
            box = [obj.xBoxSize(), obj.yBoxSize()];%    get sonnet box
            bounds = obj.getGeometryBoundingBox(); %    get total geometry bounding box
            if ~any(bounds, 'all')
                return;
            end
            boundSize = [bounds(2, 1)-bounds(1,1),bounds(2,2)-bounds(1,2)];%    get x, y size of bounding box
            centerDiff = [(box(1)-boundSize(1))/2-bounds(1), (box(2)-boundSize(2))/2-bounds(1,2)];%find distance to move all geometry
            for idx = 1:length(obj.ArrayOfPolygons)
                obj.ArrayOfPolygons{idx}.movePolygonRelative(centerDiff(1), centerDiff(2));
            end
        end
       ...
       function [aPort] = formFeedline(obj, thePolygon)
            %formFeedline(obj, thePolygon) create a feedline out of thePolygon
            %   assuming thePolygon is a four sided rectangle
            %stretches a rectangle to the edge to form a feedline
            %   !Does not create a new polygon!
            %   If the polygon is not a rectangle, it probably won't work
            %   correctly. If the polygon a sufficiently thin rectangle
            %   (even width = 0) the feedline will go to the nearest
            %   parallel wall.
            closestX = obj.xBoxSize;
            closestXi = 0;
            closestY = obj.yBoxSize;
            closestYi = 0;
            
            for idx = 1:length(thePolygon.XCoordinateValues)
                a = min(thePolygon.XCoordinateValues{idx}, abs(obj.xBoxSize-thePolygon.XCoordinateValues{idx}));
                if a < closestX
                    closestX = a;
                    closestXi = idx;
                end
                a = min(thePolygon.YCoordinateValues{idx}, abs(obj.yBoxSize-thePolygon.YCoordinateValues{idx}));
                if a < closestY
                    closestY = a;
                    closestYi = idx;
                end
            end
            if closestXi == 1 && thePolygon.XCoordinateValues{1} ~= thePolygon.XCoordinateValues{2}
                closestXi = 4;
            end
            if closestYi == 1 && thePolygon.YCoordinateValues{1} ~= thePolygon.YCoordinateValues{2}
                closestYi = 4;
            end
            %decide whether to stretch in X or Y direction
            xEval = 0;
            yEval = 0;
            if closestY == 0
                yEval = 1;
            elseif closestX == 0
                ...
                xEval = abs(thePolygon.YCoordinateValues{closestXi}-thePolygon.YCoordinateValues{closestXi+1})/(closestX*closestX);
                yEval = abs(thePolygon.XCoordinateValues{closestYi}-thePolygon.XCoordinateValues{closestYi+1})/(closestY*closestY);
            end
            if xEval>yEval
                if thePolygon.XCoordinateValues{closestXi} > obj.xBoxSize()/2
                    thePolygon.XCoordinateValues{closestXi} = obj.xBoxSize();
                    thePolygon.XCoordinateValues{closestXi+1} = obj.xBoxSize();
                    if closestXi == 4
                        thePolygon.XCoordinateValues{1} = obj.xBoxSize();
                    elseif closestXi == 1
                        thePolygon.XCoordinateValues{5} = obj.xBoxSize();
                    end
                    aPort = obj.addPortToPolygon(thePolygon, closestXi);
                    obj.addReferencePlane('RIGHT', 'LINK', thePolygon, mod(closestXi+1, 4)+1);
                else
                    thePolygon.XCoordinateValues{closestXi} = 0;
                    thePolygon.XCoordinateValues{closestXi+1} = 0;
                    if closestXi == 4
                        thePolygon.XCoordinateValues{1} = 0;
                    elseif closestXi == 1
                        thePolygon.XCoordinateValues{5} = 0;
                    end
                    aPort = obj.addPortToPolygon(thePolygon, closestXi);
                    obj.addReferencePlane('LEFT', 'LINK', thePolygon, mod(closestXi+1, 4)+1);
                end
                aPort.Type = 'BOX';
            else
                if thePolygon.YCoordinateValues{closestYi} > obj.yBoxSize()/2
                    thePolygon.YCoordinateValues{closestYi} = obj.yBoxSize();
                    thePolygon.YCoordinateValues{closestYi+1} = obj.yBoxSize();
                    if closestYi == 4
                        thePolygon.YCoordinateValues{1} = obj.yBoxSize();
                    elseif closestYi == 1
                        thePolygon.YCoordinateValues{5} = obj.yBoxSize();
                    end
                    aPort = obj.addPortToPolygon(thePolygon, closestYi);
                    obj.addReferencePlane('BOTTOM', 'LINK', thePolygon, mod(closestYi+1, 4)+1);
                else
                    thePolygon.YCoordinateValues{closestYi} = 0;
                    thePolygon.YCoordinateValues{closestYi+1} = 0;
                    if closestYi == 4
                        thePolygon.YCoordinateValues{1} = 0;
                    elseif closestYi == 1
                        thePolygon.YCoordinateValues{5} = 0;
                    end
                    aPort = obj.addPortToPolygon(thePolygon, closestYi);
                    obj.addReferencePlane('TOP', 'LINK', thePolygon, mod(closestYi+1, 4)+1);
                end
                aPort.Type = 'BOX';
            end
       end
        
        function [aPort] = formFeedlineInDirection(obj, thePolygon, direction)
            %formFeedline(obj, thePolygon) create a feedline out of thePolygon
            %   assuming thePolygon is a four sided rectangle
            %stretches a rectangle to the edge to form a feedline
            %   !Does not create a new polygon!
