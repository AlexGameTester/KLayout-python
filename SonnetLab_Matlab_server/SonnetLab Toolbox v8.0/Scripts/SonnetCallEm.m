...
function [aStatus, aMessage]=SonnetCallEm(theFilename, theOptions)
    ...
    if ispc && (contains(theOptions,'-v') || contains(theOptions,'-V'))
    ...
    elseif contains(theOptions,'-s') || contains(theOptions,'-S')
        % Parse the input string
    ...
    elseif isunix && (contains(theOptions,'-v') || contains(theOptions,'-V'))
    ...
    if ~contains(theOptions,'-l') && ~contains(theOptions,'-L')
        if isunix
    ...
    if contains(theOptions,'-c') || contains(theOptions,'-C') % If they didnt pass an x value
    ...
    if contains(path,':') %if it is an absolute path
    ...
    if ~contains(theOptions,'-t') && ~contains(theOptions,'-T')
        % If we are drawing an EM Status window
        if contains(theOptions,'-p') || contains(theOptions,'-P')
    ...
    if contains(theOptions,'-t') || contains(theOptions,'-T')
        ...
        while true
            ...
            % of matlab while the simulation is running.
            pause(1);
            ...
            while feof(aFid)==0
                ...
                if contains(aFileLine,'Analysis stopped')
                    aStatus=1;
                    aMessage='Analysis stopped';
                end
                if contains(aFileLine,'EM abnormally terminated')
                    aStatus=1;
